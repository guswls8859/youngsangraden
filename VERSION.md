...
### 변경
- **스포츠필드 PDF 출력도 슬롯 인원 합산 반영** ([reports/pdf.py](reports/pdf.py) `get_sf_data`)
  - 1.2.1에서 뷰 로직(`_sf_slot`)만 고쳤는데 PDF 출력 함수(`get_sf_data`)는 여전히 `rv_list[0]` — 첫 예약 1건만 사용
  - 매칭되는 모든 entry + reservation의 예약/입장 인원을 합산해서 출력하도록 수정
  - 테니스 시간대는 grass+hard 두 코트 예약을 함께 합산 (구분 없이 시간대별 총합)
  - entry 우선 정책 유지
